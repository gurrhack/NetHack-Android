...
{
	return(doopen_ext(FALSE, FALSE));
}

/* doopen_ext() added for additional options */
int
doopen_ext(has_dir, askkick)
boolean has_dir, askkick;
{
	coord cc;
	...
	if (!(door->doormask & D_CLOSED)) {
	    ...
	    if (askkick) {
			if (yn("The door is locked. Kick it?") == 'y') {
				dokick_ext(TRUE);
				if(!closed_door(cc.x,cc.y))
					return(1);
			}
		}
	    return(0);
